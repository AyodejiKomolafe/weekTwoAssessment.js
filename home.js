...
const canWeDeliver = (zipCode) => {
    for(let i = 0; i < deliveryAreaZipCodes.length; i++){
        if(zipCode === deliveryAreaZipCodes[i]){
            ...
            return `Sorry, we cant deliver to that address right now`
        }
    }
...
const canWeDeliverTwo = (zipCode) =>{
    if(deliveryAreaZipCodes.includes(zipCode)){
        return `Congrats, we deliver to your area`
    }else{
        return `Oops, we dont deliver to your area at the momemt`
    }
}
console.log(canWeDeliverTwo(852011))
...
deals.forEach((deal,index,wholearray) => {
